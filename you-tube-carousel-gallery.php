...
class youtubeCarouselGallery{
	...
	function __construct(){
		...
		//$this->pluginUrl = WP_PLUGIN_URL.'/you-tube-carousel-gallery/';
		...
		//if ( current_user_can( 'edit_posts' )  && current_user_can( 'edit_pages' )   && get_user_option('rich_editing') == 'true' ){
			add_filter( "mce_external_plugins", array(&$this,"YTC_add_buttons") );
			add_filter( 'mce_buttons', array(&$this,'YTC_register_buttons') );
		//}	
	}
	...
	function admin_scripts(){
		$path=$this->pluginUrl;
		wp_enqueue_style('ytc-admin-style', $path.'/css/admin-css.css');
	...
	//--------------mce-Button------------//
		...
		function YTC_add_buttons($plugin_array){
			global $tinymce_version;
			if ( version_compare( $tinymce_version, '4018' ) >= 0 ) {
				$plugin_array['YTC'] = $this->pluginUrl.'tinymce/editor_plugin.new.js';;
			}
			else{
				$plugin_array['YTC'] = $this->pluginUrl.'tinymce/editor_plugin.js';;
			}
		...
		function YTC_register_buttons($buttons){
			global $tinymce_version;
			if ( version_compare( $tinymce_version, '4018' ) >= 0 ) {
				array_push( $buttons, 'YTC' ); 
			}
	...
	function ycg_add_page(){
		add_options_page('Youtube Carousel Gallery', 'YouTube Carousel Gallery', 'manage_options', 'youtubecarouselgallery', array(&$this,'ycg_options_page'));
	}
					...
					<table class="form-table">
						<tbody>
							...
							<th scope="row"><?php _e("Choose Caption Color:", 'eg_trans_domain' ); ?></th>
								<td>
								<input type="text" class="ytc-color" name="ycg_captioncolor" value="<?php echo get_option('ycg_captioncolor'); ?>" />
								<br/>
							...
							<th scope="row"><?php _e("Choose Background Color:", 'eg_trans_domain' ); ?></th>
								<td>
								<input type="text" class="ytc-color" name="ycg_backgroundcolor" value="<?php echo get_option('ycg_backgroundcolor'); ?>" />
								<br/>
